...
templateRoutes.post('/install/:id', requireAdmin, async (c) => {
  ...
  if (templateId === 'blog') {
    ...
    install('dossiers', 'Dossiers', {
      title:           { type: 'string',   required: true,  label: 'Titel', group: 'Content', depth: 0 },
      excerpt:         { type: 'string',   required: false, label: 'Teaser', group: 'Content', depth: 0 },
      body:            { type: 'richtext', required: false, label: 'Text', group: 'Content', depth: 0 },
      humanSummary:    { type: 'string',   required: false, label: 'Human Summary', group: 'Content', depth: 0 },
      hypothesis:      { type: 'string',   required: false, label: 'Hypothese', group: 'Research', depth: 1 },
      testSetup:       { type: 'string',   required: false, label: 'Test-Setup', group: 'Research', depth: 1 },
      observations:    { type: 'string',   required: false, label: 'Beobachtungen', group: 'Research', depth: 1 },
      findings:        { type: 'string',   required: false, label: 'Findings', group: 'Research', depth: 1 },
      limitations:     { type: 'string',   required: false, label: 'Einschränkungen', group: 'Research', depth: 1 },
      openQuestions:    { type: 'string',   required: false, label: 'Offene Fragen', group: 'Research', depth: 1 },
      claims:          { type: 'table',    required: false, label: 'Claims', group: 'Evidence', depth: 2 },
      sources:         { type: 'table',    required: false, label: 'Sources (APA 7)', group: 'Evidence', format: 'apa7', depth: 2 },
      relationships:   { type: 'table',    required: false, label: 'Relationships', group: 'Evidence', depth: 2 },
      series:          { type: 'string',   required: false, label: 'Serie', group: 'Meta', depth: 1 },
      keywords:        { type: 'array',    required: false, label: 'Keywords', group: 'Meta', depth: 3 },
      contentType:     { type: 'select',   required: false, label: 'Content-Typ', group: 'Meta', depth: 1, options: ['investigation', 'analysis', 'comparison', 'guide', 'deep-dive', 'tutorial'] },
      language:        { type: 'select',   required: false, label: 'Sprache', group: 'Meta', depth: 1, options: ['de', 'en', 'de+en'] },
      author:          { type: 'string',   required: false, label: 'Autor', group: 'Provenance', depth: 3 },
      authorship:      { type: 'select',   required: false, label: 'Authorship', group: 'Provenance', depth: 3, options: ['human', 'ai-assisted', 'ai-generated', 'collaborative'] },
      aiContribution:  { type: 'string',   required: false, label: 'AI-Beitrag', group: 'Provenance', depth: 3 },
      reviewedBy:      { type: 'string',   required: false, label: 'Reviewed by', group: 'Provenance', depth: 3 },
      reviewStatus:    { type: 'select',   required: false, label: 'Review-Status', group: 'Provenance', depth: 3, options: ['draft', 'peer-reviewed', 'self-reviewed', 'unreviewed'] },
      modelDisclosure: { type: 'string',   required: false, label: 'Modell-Disclosure', group: 'Provenance', depth: 3 },
      summaryMachine:  { type: 'string',   required: false, label: 'Machine Summary', group: 'Agent', depth: 3 },
      dossierId:       { type: 'string',   required: false, label: 'Dossier-ID', group: 'Agent', depth: 3 },
      tokensApprox:    { type: 'number',   required: false, label: 'Tokens (ca.)', group: 'Agent', depth: 3 },
      suggestedPrompts:{ type: 'array',    required: false, label: 'Suggested Prompts', group: 'Agent', depth: 3 },
      hero:            { type: 'media',    required: false, label: 'Hero-Bild', depth: 0 },
    }, false, [{
